...
module.exports = function (client) {
  ...
  client.on("voiceStateUpdate", (oldState, newState) => {
    ...
    if (!oldState.channelID && newState.channelID) {
      if (newState.channelID !== '800636343838441492') return;  //if its not the jointocreatechannel skip
    ...
    if (oldState.channelID && newState.channelID) {
      ...
      if (oldState.channelID !== newState.channelID) {
        ...
        if (newState.channelID === '800636343838441492')
          //make a new channel
  ...
  async function jointocreatechannel(user) {
    ...
    await user.guild.channels.create(`${user.member.user.username}'s Room`, {
      type: 'voice',
      parent: '800636033473576990', //or set it as a category id
    }).then(async vc => {
